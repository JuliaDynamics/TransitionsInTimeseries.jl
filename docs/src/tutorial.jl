...
3. Computing the changes of the indicator by sliding a window over its timeseries. Alternatively, the change metric can be estimated over the whole segment, as examplified in the section [Segmented windows](@ref).
...
## [Tutorial -- Convenient sliding window] (@id example_fastforward)
...
and the function [`estimate_significance!`](@ref). Finally, we can
...
flags = significant_transitions(results, signif)
tv = plot_indicator_changes(results, additional_timeseries = x_nlinear[2:end])
plot_significance!(tv, signif, flags = flags, nsurro = 100)
tv.fig