...
"""
function precompute_metrics(metrics::AbstractVector, t::AbstractVector)
    f = Function[]
    for metric in metrics
        if typeof(metric) <: PrecomputableFunction
            push!(f, precompute(metric, t))
        elseif isa(metric, Function)
            push!(f, metric)
        else
            error("The function $metric has an erroneous type (not a function).")
        end
    end
    return f
end
