...
Computes the `evolution_metric` of an `indicator` for a timeseries (`t`, `x`) and
`n_surr` surrogates generated by `surr_method`. The windowing parameters for the
computation of the `indicator` are `wv_indicator_width` and `wv_indicator_stride`.
The windowing parameters for the computation of the `evolution_metric`, are
`wv_evolution_width` and `wv_evolution_stride`.

Future dev: take vector of indicators such that surrogates need to be generated only once.
"""
