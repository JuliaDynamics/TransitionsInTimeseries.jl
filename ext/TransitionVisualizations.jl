...
global default_accent_linewidth = 3
global default_colors = ["#7143E0", "#0A9A84", "#191E44", "#AF9327", "#701B80", "#2E6137"]
...
function TransitionsInTimeseries.plot_indicator_changes(res::SlidingWindowResults;
    ...
    return TransitionVisualization(fig, laxs, raxs, res)
end
...
function init_rowwise_visualisation(res, colors, indicator_names,
    ...
    labels = ["original signal", "surrogate signals"]
    width = 0.5
    if length(res.t_indicator[1]) > 1
        ...
        labels = vcat(labels, ["original change metric", "surrogate change metrics"])
        width = 1
    end
...
function TransitionsInTimeseries.plot_significance!(
    ...
    config = res.config
    for k in eachindex(res.t_indicator)
        lines_over_surro!(raxs, laxs, nsurro, res.t[res.i1[k]:res.i2[k]], res.t_indicator[k],
            res.t_change[k], res.x[res.i1[k]:res.i2[k]], signif, config, flags[k, :], colors)
    end
    return tv
end
...
function plot_changes_significance(res, signif)
